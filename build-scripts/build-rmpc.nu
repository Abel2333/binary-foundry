...
def main [
  ...
  let manifest = (asset_config "rmpc")
  install_rust_deps []

  let repo_dir = ([$work_dir "rmpc"] | path join)
  ...
  run_rust_build $repo_dir [build --release --bin rmpc]

  install_stage_file $"($repo_dir)/target/release/rmpc" $"($stage_dir)/usr/bin/rmpc"
